...
The current Pro EVM `PythLazer.verifyUpdate` verifies one trusted ECDSA signer per supplied envelope and returns its payload and signer. Do not describe one successful Pro verification as a three-router quorum. Any additional independent-router requirement needs an explicit consumer design and Pyth agreement. [Official verifier source](https://github.com/pyth-network/pyth-crosschain/blob/main/lazer/contracts/evm/src/PythLazer.sol).
...
- Consumer account: the selected Pro route requires an entitled backend Pro API token, including permission for signed EVM payloads. The offchain monitor's parsed-only output is insufficient for transactions. Hermes is a **Core alternative**: it now requires an API key at the upgraded endpoint `https://pyth.dourolabs.app/hermes`. Core availability and feed mappings require separate confirmation. [Pro REST API](https://docs.pyth.network/price-feeds/pro/api/rest), [Core upgrade procedure](https://docs.pyth.network/price-feeds/core/upgrade/preparing).
...
- Onchain updater: the selected Pro route needs its own signed-payload consumer and transaction service, RPC access, gas signer, approved official verifier and operating budget. The official **Core Price Pusher** is a different integration; it requires v10.5.0 or later for Hermes authentication and is not the numeric-feed Pro updater. Neither route can create or list SBX through a transaction alone. [Pro integration](https://docs.pyth.network/price-feeds/pro/integrate-as-consumer/evm), [Core Price Pusher](https://docs.pyth.network/price-feeds/core/schedule-price-updates/using-price-pusher).
