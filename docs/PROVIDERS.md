...
| `runpod-secure` | `RUNPOD_API_KEY` | Lowest advertised Secure Cloud rate for the one-GPU query | Qualitative stock plus nullable deployment sizes | Read-only pricing schema verified; disabled by default; collection and publication rights still required |
...
The [official schema](https://graphql-spec.runpod.io/#definition-LowestPrice) permits `availableGpuCounts: null` and `stockStatus: null`. A read-only check on September 6, 2026 returned null sizes with `Low` stock for B200 and B300. Null does not document an empty inventory: the collector retains the valid advertised price with `UNKNOWN` availability and no inferred minimum order. Null stock is also unknown unless an explicit size list excludes one GPU. `None` stock remains unavailable. Absent fields, malformed values and null list entries still fail closed. This validates a response shape, not allocation, complete key permissions or publication rights; the collector remains disabled by default.
