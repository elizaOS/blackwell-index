...
| Production operations | Hosted recurrence and identity/history persistence verified; signed private hosted export and encrypted recovery tools implemented | Verify recovery for the deployed revision; assign offsite destination, separate key custodian, paging and retention owners; plan streaming archives beyond the 8 MiB export cap and run a host-loss/rotation exercise |
