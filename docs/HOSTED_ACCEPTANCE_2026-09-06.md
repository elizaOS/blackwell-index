...
V1 keeps its original 8 MiB logical-export and whole-database limits. V2 removes that total-buffer dependency for the tested workload; it does not create unlimited retention. Local re-backup was missing at the hosted revision recorded above; the subsequent `backup-stream` implementation is described in the [current runbook](STREAMING_RECOVERY.md) and requires its own exact-revision checks. Confirm the account-specific storage/CPU allowance, monitor growth and existing report limits, test ongoing backups before activating a restored node, and complete offsite/key-custody/host-loss operations. Do not delete evidence or counters to fit a cap. The [streaming recovery plan](STREAMING_RECOVERY_PLAN.md) retains these acceptance boundaries.
