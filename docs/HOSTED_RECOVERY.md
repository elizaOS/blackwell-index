...
Cloudflare's built-in point-in-time recovery must not be used to resume a rolled-back signer with its old counter. Coordinate revocation and key rotation first. Its [SQLite storage documentation](https://developers.cloudflare.com/durable-objects/api/sqlite-storage-api/) describes PITR separately from application-level export. The helper follows Cloudflare's [Wrangler Node.js API](https://developers.cloudflare.com/workers/wrangler/api/) and [RPC service-binding model](https://developers.cloudflare.com/workers/runtime-apis/rpc/).
