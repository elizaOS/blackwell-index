...
The separate [Pyth integration](PYTH.md) includes a conformance test against the actual pinned official Rust agent. Local queue acknowledgement is not Pyth publication or settlement proof. Run `bun test test/recovery.test.ts test/collection-control.test.ts test/hosted-export.test.ts test/cli-study.test.ts` for recovery, throttling, export and CLI regressions. The [recovery procedure](RECOVERY.md) and [hosted export guide](HOSTED_RECOVERY.md) state backup limits and remaining operational responsibilities.

The hosted export review identified and addressed graceful interruption cleanup, raw parser-error disclosure, SQLite's permissive column affinity, missing local observation evidence, and explicit exclusion of Cloudflare's private KV tables. Export access uses a private named service-binding entrypoint; no public HTTP handler dispatches to it. Fixed table schemas and typed values are checked before materialization, and signed export identity/revision are pinned. Tests include controlled SIGTERM after plaintext materialization and an actual workerd service-binding export, encrypted import, inspection and isolated restore. SIGKILL/host loss and large-journal archive capacity remain operational limitations, not solved by these tests.
