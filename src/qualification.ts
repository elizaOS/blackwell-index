...
export function qualifyModel(batches: SignedBatch[], registry: Registry, methodology: Methodology, model: GpuModel, at: number) {
  ...
  const groups = Object.keys(weights).sort().map(group => {
    const providers = registry.providers.filter(value => value.economicGroup === group);
    return { group, weight: weights[group]!, providers: providers.map(value => value.id).sort(),
      rightsConfigured: providers.some(value => value.rights.collect && value.rights.redistribute && value.rights.derive &&
        Boolean(value.rights.evidence.trim()) && (value.rights.expiresAt === null || value.rights.expiresAt > at)),
      matchedReportsReady: providers.some(value => snapshot.feeds.some(item => item.id === `SBX:${value.id}:${model}` && item.status === "READY")) };
  });
  ...
  if (!snapshot.publishable) blockers.push("CURRENT_COMPOSITE_PUBLICATION_GATE_NOT_MET");
